...
export class QiitaPresenter {
  ...
  private setSubject(): void {
    ...
    const nodes = this.dest.querySelectorAll<HTMLElement>('.js-subject')
  ...
  private createArticleFragment(
    ...
    Object.entries<unknown>(content).forEach((kv): void => {
  ...
  private fillTags(tags: QiitaResponse.Tag[], template: HTMLElement) {
    tags.forEach((tag) => {
